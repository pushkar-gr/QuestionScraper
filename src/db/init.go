...
// input: Config
// output: error if any
func Init(config *types.Config) error {
	...
	for _, platform := range config.Platforms {
		...
		var existingID int
		err := db.QueryRow("SELECT id FROM platforms WHERE name = $1", platform.Name).Scan(&existingID)
		if err != nil {
			if err == sql.ErrNoRows {
				//platform does not exist
				_, err := db.Exec("INSERT INTO platforms (name, website_url) VALUES ($1, $2)", platform.Name, platform.WebsiteURL)
				if err != nil {
	...
	for _, topic := range config.Topics {
		...
		var existingID int
		err := db.QueryRow("SELECT id FROM topics WHERE name = $1", topic.Name).Scan(&existingID)
		if err != nil {
			if err == sql.ErrNoRows {
				//topic does not exist
				_, err := db.Exec("INSERT INTO topics (name, description) VALUES ($1, $2)", topic.Name, topic.Description)
				if err != nil {
